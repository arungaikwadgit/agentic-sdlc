...
- **Continuous uptime monitoring — user to set up, not yet done.** UptimeRobot free tier chosen (50 monitors, 5-min interval, email/Slack alerts). Requires an account the user creates themselves (account creation is outside what I can do); I can help configure the 3 monitor URLs once it exists. This catches drift *between* deploys — the actual failure mode behind item #4/#5 Phase 3 sitting broken for weeks with zero pushes in that window, which a deploy-triggered smoke test structurally cannot catch.
