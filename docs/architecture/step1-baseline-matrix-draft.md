...
| Project/Admin API (`server/src`) | 🟡 (live and serving traffic, but its CI/CD is broken) | Deployed as Railway service `artistic-charm` (project `zucchini-rejoicing`), `rootDirectory: "server"`, currently live on commit `293c48c1` (2026-08-07), healthy `/health` check | **Every deploy of this service has been silently failing since at least Aug 7** — repo-root `railway.json` (meant for the other service) is shadowing this service's own build config, so `tsc` never runs and `dist/index.js` never gets produced; Railway's rollback masked it with zero visible outage. Root-caused this session; fix (`server/railway.json`) written, committed, pending push + redeploy. **No server-side code change has actually reached production since Aug 7**, regardless of what's merged to `main` — this matters for any other row in this matrix that assumes recent `server/` commits are live | Deploy logs for failures `63885eb1` and `3ccd8371`; `get-service-config` on both Railway services; repo-root `railway.json` vs. missing `server/railway.json` | High — was previously misclassified as simply "live, healthy." It IS live, but its deployment pipeline has been dark for ~2 weeks. Any Step 6 spec assuming a recent `server/` fix is running in production needs a redeploy-and-verify step first |
