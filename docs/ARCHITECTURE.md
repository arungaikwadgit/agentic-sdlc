...
Deployment requires migration `009_background_agent_lifecycle.sql`. The proxy service requires `RUNTIME_API_URL` and `RUNTIME_API_TOKEN`; the runtime requires `PROXY_API_URL` and `PROXY_TOKEN` to reach back to the proxy for the actual LLM call (opt-out, not opt-in: the worker runs by default and only skips starting if these are missing, or if `BACKGROUND_WORKER_ENABLED` is explicitly set to `false` — this doc previously said `=true` was required, which didn't match `lifecycleWorker.ts`'s actual gate).

**2026-08-24 note:** `PROXY_API_URL`/`PROXY_TOKEN` were missed when `agentic-sdlc-runtime` was provisioned earlier the same day (Section "Deploy Verification & Monitoring" above covers `RUNTIME_API_URL` going the other direction, but not this leg) — the worker sat silently disabled until caught and fixed later that day. See `docs/architecture/execution-status-2026-08-24.md` Section 6 for the full incident detail.
